...
export abstract class BaseFileController implements FileController {
    protected fs = new VSCodeFileSystem();

    constructor(protected context: ExtensionContext) {}

    public abstract showDialog(options?: DialogOptions): Promise<VsCodeFileItem | VsCodeFileItem[] | undefined>;

    public abstract execute(options: ExecuteOptions): Promise<VsCodeFileItem>;

    private get pathTypeIndicator(): string {
        return getConfiguration("inputBox.pathTypeIndicator") ?? "";
    }

    public async openFileInEditor(fileItem: VsCodeFileItem): Promise<TextEditor | undefined> {
        if (fileItem.isDir) {
            return;
        }

        const textDocument = await workspace.openTextDocument(fileItem.loc.path);
        if (!textDocument) {
    ...
    protected async ensureWritableFile(fileItem: VsCodeFileItem): Promise<VsCodeFileItem> {
        ...
        if (fileItem.targetLoc === undefined) {
